...
const buildLogo = (wrapper, logoData) => {
    ...
    if (logoData.shapeName == "oval") {
        ...
        shape = wavyOval({
            width: parseInt(adjustToContent) ? textBox.width/Math.sqrt(2) * 2 : diagonal + 25,
            height: parseInt(adjustToContent) ? textBox.height/Math.sqrt(2) * 2  : diagonal + 25,
            type: type,
            steps: steps
        })
        shape.setAttribute("transform", "translate(150 150)")
        shape.setAttribute("fill", parseInt(fill) ? "black" : "transparent")
    ...
    if (logoData.shapeName = "textDecoration") {
        let [step, height, strokeWidth, bottom, top] = logoData.args
        ...
        if (parseFloat(top)) {
            ...
            shapeTop.setAttribute("transform", `translate(${150 - shapeTop.getBBox().width/2} ${textBox.y - shapeTop.getBBox().height - strokeWidth * 2})`)      
        }
        if (parseFloat(bottom)) {
            const shapeBottom = shape.cloneNode()
            svg.appendChild(shapeBottom)
            shapeBottom.setAttribute("transform", `translate(${150 - shapeBottom.getBBox().width/2} ${textBox.y + textBox.height + shapeBottom.getBBox().height + strokeWidth * 2})`)      
        }
    }
